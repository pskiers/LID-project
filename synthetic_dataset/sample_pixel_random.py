"""
sample_pixel_random.py
----------------------
Generate images from random conditioning vectors and save as individual PNG files.
Runs in batches, prints ETA after first batch.

Usage:
    python sample_pixel_random.py --checkpoint outputs_64/checkpoint-epoch-0200
    python sample_pixel_random.py --checkpoint outputs_64/checkpoint-epoch-0200 --n 10000 --batch_size 64
"""
...
def parse_args():
    ...
    parser.add_argument("--n",          type=int, default=10000,  help="Total number of images to generate")
    parser.add_argument("--batch_size", type=int, default=64,    help="Images per batch")
    parser.add_argument("--seed",       type=int, default=42)
    parser.add_argument("--out_dir",    type=str, default="outputs/samples/samples_64_random_random")
    return parser.parse_args()


def random_conditions(n, seed):
    ...
    for _ in range(n):
        ...
        v_stripe = rng.random()
        grain    = rng.random()
        conds.append(one_hot + [r, g, b, size, h_stripe, v_stripe, grain])
    return conds
...
def main():
    ...
    all_conds = random_conditions(args.n, args.seed)
    n_batches = (args.n + args.batch_size - 1) // args.batch_size

    np.save(out_dir / "vectors.npy", np.array(all_conds, dtype=np.float32))
    print(f"Saved conditioning vectors → {out_dir / 'vectors.npy'}  shape: ({args.n}, 10)")
    ...
    for batch_idx in range(n_batches):
        ...
        batch_conds = torch.tensor(all_conds[start:end], dtype=torch.float32)
